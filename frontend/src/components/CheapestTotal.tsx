...
export default function CheapestTotal({ outboundPrice, inboundPrice, currency, passengers = 1, isCustomSelection }: Props) {
  ...
  const label = isCustomSelection ? 'Selected total' : 'Cheapest total'
  const passengerLabel = passengers > 1 ? `for ${passengers} passengers` : 'per person'
  ...
  return (
    <div className="bg-white rounded-2xl border border-brand-200 ring-1 ring-brand-100 px-6 py-4 flex items-center justify-between dark:bg-gray-900 dark:border-brand-800 dark:ring-brand-900">
      <div className="text-sm text-gray-500 dark:text-gray-400 space-y-0.5">
        {outboundPrice !== null && (
          <p>Outbound: <span className="font-medium text-gray-700 dark:text-gray-200">{currency} {outboundPrice.toFixed(2)}</span> <span className="text-gray-400 dark:text-gray-500">per person</span></p>
        )}
        {inboundPrice !== null && (
          <p>Return: <span className="font-medium text-gray-700 dark:text-gray-200">{currency} {inboundPrice.toFixed(2)}</span> <span className="text-gray-400 dark:text-gray-500">per person</span></p>
        )}
      </div>
      <div className="text-right">
        ...
        {passengers > 1 && (
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{currency} {perPersonTotal.toFixed(2)} per person</p>
        )}
      </div>
    </div>
  )
